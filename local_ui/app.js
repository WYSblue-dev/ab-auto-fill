...
function notice(message, error=false, target='notice') {
  ...
  if (message && error) node.scrollIntoView({block:'nearest'});
}
...
function renderReview(held=records(['review','uncertain'])) {
  ...
  const created = row.pending_receipt;
  const lookup = created ? null : row.lookup;
  const lookupLabel = created ? 'Person created · verification pending' : !lookup ? 'Check required' : lookup.outcome==='not_found' ? (row.status==='uncertain'?'Saved contact check: no matches':'No email or phone matches') : (row.status==='uncertain'?'Contact match found · submission still unconfirmed':'Possible existing person');
  $('review-layout').innerHTML = `<div class="review-grid"><aside class="people-list" aria-label="People needing review">${held.map(person=>`<button class="person-button ${person.id===row.id?'selected':''}" data-person="${person.id}" aria-pressed="${person.id===row.id}"><strong>${escapeText(fullName(person))}</strong>${pill(person.pending_receipt?'Person created · verify':person.status==='uncertain'?'Check earlier attempt':'Needs review','warn')}</button>`).join('')}</aside><div class="review-detail"><h3>${escapeText(fullName(row))}</h3><div class="columns"><section class="card"><h3>From the paperwork</h3><div id="contact-detail"><dl>${state.fields.map(([key,label])=>`<dt>${escapeText(label)}</dt><dd>${escapeText(row.person[key] || 'Not set')}</dd>`).join('')}</dl><div class="action-bar"><button id="edit">Edit these details</button></div></div><form id="edit-form" hidden><div class="edit-fields">${state.fields.map(([key,label])=>`<label>${escapeText(label)}${key==='classification'?`<select name="classification" ${state.settings.residence_local?'required':''}><option value="">Choose a classification</option>${state.classifications.map(name=>`<option ${name===row.person.classification?'selected':''} value="${escapeText(name)}">${escapeText(name)}</option>`).join('')}</select>`:`<input name="${key}" value="${escapeText(row.person[key])}" required>`}</label>`).join('')}</div><div class="action-bar"><button type="button" id="edit-cancel">Cancel</button><button type="submit" class="primary">Save corrections</button></div></form></section><section class="card"><h3>Action Builder check</h3>${pill(lookupLabel, lookup?.outcome==='not_found' && row.status!=='uncertain'?'good':'warn')}${created?`<p>Action Builder returned a person receipt. Verification could not complete. Use the saved person ID to check the existing entry.</p><code>${escapeText(created.identifiers.join(', '))}</code><p>Created in ${escapeText(created.destination.subdomain)} · Campaign ${escapeText(created.destination.campaign_id)}</p>`:''}${row.status==='uncertain' && lookup && lookup.outcome!=='not_found'?'<p>A matching entry may be the result of the earlier sending attempt. Completion still requires verification.</p>':''}<p>${escapeText(lookup?.reason || row.reason)}</p>${lookup?`<p>Checked in ${escapeText(lookup.destination.subdomain)} · Campaign ${escapeText(lookup.destination.campaign_id)}</p>`:''}${(lookup?.candidates || []).map(candidate=>`<div class="candidate"><strong>Candidate record</strong><code>${escapeText(candidate.identifiers.join(', '))}</code><p>Matches: ${escapeText(candidate.matching_fields.map(fieldLabel).join(', '))}</p><p>Differs: ${escapeText(candidate.differing_fields.map(fieldLabel).join(', ') || 'None')}</p></div>`).join('')}${row.related_sent?'<div class="note">A related record was already submitted. Creating this person again is blocked.</div>':''}${row.related_created?'<div class="note">Action Builder already returned a person receipt for this record or a related version. Creating another person is blocked.</div>':''}${row.related_uncertain && !row.related_created?'<div class="note">An earlier attempt may have created this person. Check Action Builder before approving another submission.</div>':''}<div class="action-bar"><button id="review-check">Check revised details</button></div></section></div><p class="small source">${row.source_names.map(escapeText).join(' · ')}</p><div class="note">${escapeText(row.reason)}</div><div class="action-bar"><div><button id="keep">Keep for later</button><button id="discard" class="danger">Discard this import</button>${row.status==='uncertain'?`<button id="reconcile">${created?'Verify created person':'Mark already created'}</button>`:''}</div><button class="primary" id="approve" ${row.related_sent || row.related_created || !checkedIds.has(row.id) || !state.settings.configured ? 'disabled':''}>Review sending as a new person</button></div><p class="small">${created?'Verify created person checks the saved ID without creating another person.':'Run a fresh check above to enable approval. Submission checks again before sending.'}</p></div></div>`;
  document.querySelectorAll('[data-person]').forEach(button=>button.onclick=()=>{selectedId=button.dataset.person;renderReview();});
  $('edit').disabled = Boolean(row.related_created);
  if(row.related_created)$('edit').insertAdjacentHTML('afterend','<p class="small">Verify the created person before editing this import.</p>');
  ...
  $('review-check').hidden = Boolean(created);
...
function confirmAction(kind,row) {
  ...
  const created = kind==='reconcile' && row.pending_receipt;
  ...
  $('ack-copy').textContent=created?'I reviewed the saved person receipt and want to verify this existing Action Builder entry.':kind==='reconcile'?'I verified that this person already exists with the correct contact details in this campaign.':'I checked the existing records and any earlier sending attempt, and confirmed this person should be created as a new person.';
  $('confirm-title').textContent=created?'Verify the created person?':kind==='discard'?'Discard this import?':kind==='reconcile'?'Mark the earlier submission complete?':'Confirm new-person submission';
  ...
  $('confirm-submit').textContent=created?'Verify created person':kind==='discard'?'Discard import':kind==='reconcile'?'Verify and mark complete':'Confirm submission';
  const names=kind==='batch'?records(['pending']).map(fullName):[fullName(row)];
  $('confirm-content').innerHTML=`<ul>${names.map(name=>`<li>${escapeText(name)}</li>`).join('')}</ul><p>${kind==='batch'?'Fresh checks run for everyone. Only people with no email or phone matches are submitted; others stay held.':kind==='reconcile'?`${created?'A read-only check uses the saved Action Builder person ID.':'A read-only check must find one exact contact match.'}${state.settings.residence_local?' It must also confirm the classification tag, the configured residence-local tag, and assessment 1.':''} Only your local queue is updated; nothing is sent again.`:kind==='review'?'This creates a new person. It does not update an existing record.':'The local contact file will be removed. This action cannot be undone in the app.'}</p>`;
...
$('confirm-form').onsubmit=async event=>{
  ...
  else if(c.kind==='reconcile'){checkedIds.delete(c.row.id);await run('review-reconcile',{id:c.row.id,destination:c.destination,residence_local:c.residence_local,checked,confirmed:true});}
